...
public class BarGraph extends View implements HoloGraphAnimate {
    ...
    private Paint mPopUpPaint = new Paint();
       ...
       // NinePatchDrawable popup = (NinePatchDrawable) resources.getDrawable(R.drawable.popup_black);
        ...
        for (final Bar bar : mBars) {
            ...
            if (mShowBarText) {
                ...
                if (mShowPopup) {
                    mPopUpPaint.setColor(getContext().getResources().getColor(R.color.black));
                    mPopUpPaint.setAntiAlias(true);
                    mPopUpPaint.setStyle(Paint.Style.FILL);
                    mPopUpPaint.setAlpha(70);
                    if (isAnimating()) mPopUpPaint.setAlpha(popupAlpha);
                    canvas.drawRect(boundLeft, boundTop, boundRight, mBoundsRect.top-10, mPopUpPaint);
